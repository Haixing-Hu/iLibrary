  ...
  public void prepareForOpen() {
    ...
    if (showTitle) {
      ...
      if (systemControlsBar == null && (style & SWT.CLOSE) != 0) {
        ...
        final Image closeImage = createCloseImage(shell.getDisplay(), closeBG,
            closeFG);
        shell.addListener(SWT.Dispose, new Listener() {
          public void handleEvent(Event event) {
            closeImage.dispose();
    ...
    final Point shellSize = (anchor == SWT.NONE) ? new Point(contentsSize.x
        + marginLeft + marginRight, contentsSize.y + marginTop + marginBottom)
        : new Point(contentsSize.x + marginLeft + marginRight, contentsSize.y
