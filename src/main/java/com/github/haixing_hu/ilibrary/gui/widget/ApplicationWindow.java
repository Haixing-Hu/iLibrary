 ...
 * </p>
 * <p>
 * <b>NOTE:</b> this is a copy of the {@link org.eclipse.jface.window.ApplicationWindow},
 * except that it use our modified {@link MenuManager}.
 * </p>
 *
 * @author Haixing Hu
 */
public class ApplicationWindow extends Window implements IRunnableContext {
    ...
    /*package*/class ApplicationWindowLayout extends Layout {
        ...
        return new Point(wHint, hHint);
            ...
            final Control[] ws = composite.getChildren();
            for (int i = 0; i < ws.length; i++) {
                final Control w = ws[i];
                ...
                if (!hide) {
                    final Point e = w.computeSize(wHint, hHint, flushCache);
        ...
        protected void layout(Composite composite, boolean flushCache) {
            ...
            for (final Control w : ws) {
                if (w == seperator1) { // Separator
                    final Point e = w.computeSize(SWT.DEFAULT, SWT.DEFAULT,
                            flushCache);
                    ...
                    if (coolBarChildrenExist()) {
                        final Point e = w.computeSize(clientArea.width, SWT.DEFAULT,
                                flushCache);
                        w.setBounds(clientArea.x, clientArea.y,
                                clientArea.width, e.y);
                        clientArea.y += e.y + VGAP;
   ...
   * @return the cool bar manager, or <code>null</code> if
    ...
    protected boolean toolBarChildrenExist() {
        final Control toolControl = getToolBarControl();
