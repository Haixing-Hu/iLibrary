...
import cn.edu.nju.starfish.ilibrary.utils.xml.InvalidXmlNodeContentException;
...
public final class DomainSuffixRegistry {
   ...
   * <td>String</td>
   * <td>1</td>
   * <td>the name of the XML resource file of the {@link DomainSuffixRegistry}
   * class.</td>
   * <td>no</td>
   ...
   */
  public static final String PROPERTY_RESOURCE = "com.ascentdimension.net.DomainSuffixRegistry.resource";
  ...
  public static DomainSuffixRegistry getInstance() {
    ...
    if (instance == null) {
      synchronized (DomainSuffixRegistry.class) {
        if (instance == null) {
          instance = new DomainSuffixRegistry(DEFAULT_RESOURCE);
        }
      }
    }
    return instance;
  ...
  public DomainSuffixRegistry(final String resource) {
    domains = new HashMap<String, DomainSuffix>();
    final URL configResourceUrl = ResourceUtils.getResource(resource,
        DomainSuffixRegistry.class);
    if (configResourceUrl == null) {
      LOGGER.error("Can't find the domain suffixes configuration file: {}.", resource);
    } else {
  ...
  private TopLevelDomain parseGtld(final Element tldNode,
    ...
    final String domain = getReqStringAttr(tldNode, DOMAIN_ATTRIBUTE, true,
        false);
  ...
  private TopLevelDomain parseCctld(final Element cctldNode)
      throws XmlException {
    final String domain = getReqStringAttr(cctldNode, DOMAIN_ATTRIBUTE, true,
        false);
  ...
  private DomainSuffix.Status parseStatus(final Element tldNode)
    ...
    final String statusName = getReqString(statusNode, null, true, false);
    try {
  ...
  private String parseDescription(final Element tldNode) throws XmlException {
    final Element descriptionNode = getOptChild(tldNode, DESCRIPTION_NODE);
  ...
  private DomainSuffix parseSuffix(final Element suffixNode)
      throws XmlException {
    final String domain = getReqStringAttr(suffixNode, DOMAIN_ATTRIBUTE, true,
        false);
