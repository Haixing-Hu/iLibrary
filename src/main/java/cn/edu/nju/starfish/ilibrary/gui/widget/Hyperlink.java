...
public final class Hyperlink extends Canvas {
  ...
  public Hyperlink(Composite parent, int style) {
    ...
    Listener mouseListener = new Listener() {
      public void handleEvent(Event event) {
        ...
        if (cursorInControl != newCursorInControl) {
          ...
          if (isActive || (normalForeground != hoverForeground)
              || (normalUnderline != hoverUnderline))
            redraw();
        }
      }
    };
    ...
    if (shell != null) {
      shellListener = new Listener() // Remove stale mouse hover on shell
  ...
  private boolean isInClientArea(Event event) {
    return event.x >= cachedClientArea.x
        && event.x < cachedClientArea.x + cachedClientArea.width
        && event.y >= cachedClientArea.y
  ...
  private static int checkStyle(int style) {
     ...
     * but a texture. To show the correct default background, we must allow the
     * operating system to draw it and therefore, we can not use the
  ...
  private void onPaint(PaintEvent event) {
    ...
    gc.drawText(text, rect.x, rect.y + (rect.height - textHeight) / 2,
        SWT.DRAW_TRANSPARENT | SWT.DRAW_MNEMONIC);

    int uy = (rect.y + (rect.height - textHeight) / 2)
        + gc.getFontMetrics().getAscent() + gc.getFontMetrics().getLeading()
        + 1;
   ...
   * mnemonic, a selection event occurs. On most platforms, the mnemonic appears
   * underlined but may be emphasised in a platform specific manner. The
   * mnemonic indicator character '&amp' can be escaped by doubling it in the
   * string, causing a single '&amp' to be displayed.
   * </p>
   * 
   * @param text
